...
void *comparator_routine(void *arg) {
    ...
    while (1) {
        mutex_lock(&storage->sync);

        Node *curr = storage->first;
        if (!curr) {
            mutex_unlock(&storage->sync);
            perror("first element in storage is NULL");
            break;
        }

        mutex_lock(&curr->sync);

        mutex_unlock(&storage->sync);

        Node *next = curr->next;
        while (next) {
            mutex_lock(&next->sync);

            process_operation(ttype, curr, next);

            mutex_unlock(&curr->sync);
...
void *swapper_routine(void *arg) {
    ...
    while (1) {
        ...
        mutex_lock(&first->sync);

        Node *second = first->next;
        mutex_lock(&second->sync);
        ...
        while (third) {
            mutex_lock(&third->sync);

            if (rand() % 2) {
                swap_nodes(first, &second, &third);
            }

            mutex_unlock(&first->sync);

            first = second;
            second = third;
            third = third->next;
        }
        mutex_unlock(&second->sync);
        mutex_unlock(&first->sync);
    }
...
int main(void) {
    Storage *storage = storage_create(100);
    if (!storage) {
