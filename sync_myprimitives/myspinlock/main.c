...
void *comparator_routine(void *arg) {
    ...
    while (1) {
        spin_lock(&storage->sync);

        Node *curr = storage->first;
        if (!curr) {
            spin_unlock(&storage->sync);
...
void *swapper_routine(void *arg) {
    ...
    while (1) {
        ...
        spin_lock(&first->sync);

        Node *second = first->next;
        spin_lock(&second->sync);
        ...
        while (third) {
            spin_lock(&third->sync);

            if (rand() % 2) {
                swap_nodes(first, &second, &third);
            }

            spin_unlock(&first->sync);

            first = second;
            second = third;
            third = third->next;
        }
        spin_unlock(&second->sync);
        spin_unlock(&first->sync);
    }
